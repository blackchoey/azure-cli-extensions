...
class DeploymentCommandsTests(ScenarioTest):
    ...
    def test_deployment_list_with_all_optional_params(self, deployment_id1):
        ...
        self.cmd('az apic api deployment list -g {rg} -s {s} --api-id {api} --filter "name eq \'{deployment_id}\'"', checks=[
            self.check('length(@)', 1),
            self.check('[0].name', deployment_id1)
        ])
    ...
    def test_examples_create_deployment(self):
        ...
        self.cmd('az apic api deployment create -g {rg} -s {s} --deployment-id {name} --title "Production deployment" --description "Public cloud production deployment." --api-id {api} --environment-id "/workspaces/default/environments/{e}" --definition-id "/workspaces/default/apis/{api}/versions/{v}/definitions/{d}" --server \'{server}\'', checks=[
            self.check('name', '{name}'),
    ...
    def test_examples_delete_api_deployment(self):
        self.cmd('az apic api deployment delete -g {rg} -s {s} --deployment-id {dep} --api-id {api} --yes')
        self.cmd('az apic api deployment show -g {rg} -s {s} --api-id {api} --deployment-id {dep}', expect_failure=True)
    ...
    def test_examples_update_api_deployment(self):
        self.cmd('az apic api deployment update -g {rg} -s {s} --deployment-id {dep} --title "Production deployment" --api-id {api}', checks=[
            self.check('title', 'Production deployment'),
        ])